...
impl Renderer2D {
    ...
    fn interpolate(a: Vector2<i32>, b: Vector2<i32>, t: f32) -> Vector2<i32> {
        let ax = a.x as f32;
        let ay = a.y as f32;
        let bx = b.x as f32;
        let by = b.y as f32;
...
impl Renderer for Renderer2D {
    ...
    fn fill_triangle(&mut self, v1: Vector2<i32>, v2: Vector2<i32>, v3: Vector2<i32>, color: u32) {
        ...
        for i in 0..total_height {
            ...
            let va = Renderer2D::interpolate(a, c, alpha);
            let vb = if second_half {
                Renderer2D::interpolate(b, c, beta)
            } else {
                Renderer2D::interpolate(a, b, beta)
            };
        
            let (start_x, end_x) = if va.x > vb.x { (vb.x, va.x) } else { (va.x, vb.x) };
            for x in start_x..=end_x {
                self.draw_pixel(Vector2::new(x, a.y + i), color);
            }
        }
    }
